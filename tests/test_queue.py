...
async def test_join_queue__expects_create_new_game(user: Client):
    ...
    assert response.json()["game"] == {'id': 1, 'is_finished': False, 'is_full': False, 'players': [1]}


async def test_join_queue__expects_join_already_created_game(user: Client, db: Database):
    ...
    response = await user.post("/queue/online", json={})
    assert response.status_code == 200
    assert response.json()["game"] == {'id': 1, 'is_finished': False, 'is_full': True, 'players': [1, 2]}
