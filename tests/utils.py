...
async def create_test_client(
        app, settings, db, password: str = "test123456",
        ip_address: str = "127.0.0.1",
):
    new_user = await create_random_user(db, password=password)
    await db.session.commit()
    return Client(app=app, user=new_user, settings=settings, password=password, ip_address=ip_address)
...
async def create_login_session(db: Database, anon_user: Client, password: str = 'test123454656243',
                               user: User = None):
    if not user:
        user = await create_random_user(db, password=password)
        await db.commit()
    response = await anon_user.post('/auth/login', json={'username': user.name, 'password': password})
